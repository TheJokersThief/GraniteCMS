...
{
    $menu = \App\MenuItem::where('parent', 0)->where('name', $name)->where('site', getCurrentSiteID())->first();
    if ($menu != null) {
        return $menu;
    } else {
...
{
    $menu = \App\MenuItem::where('parent', 0)->where('id', $id)->where('site', getCurrentSiteID())->first();
    if ($menu != null) {
        return $menu;
    } else {
        throw new \Exception(errorMsg('menus_no_menu'));
